...
diffsed = marine_distribution(elevation,seavol,sealevel,border,depids,pyslp,[pydnodes,pyids,pyrocknb])\n\nWrapper for ``marine_distribution``.\
\n\nParameters\n----------\n"
...
static PyObject *f2py_rout_pdalgo_marine_distribution(const PyObject *capi_self,
                           ...
                           void (*f2py_func)(double*,double*,double*,int*,int*,double*,double*,int*,int*,int*)) {
...
/*decl*/
  ...
  static char *capi_kwlist[] = {"elevation","seavol","sealevel","border","depids","pyslp","pydnodes","pyids","pyrocknb",NULL};
...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOOO|OOO:pdalgo.marine_distribution",\
    capi_kwlist,&elevation_capi,&seavol_capi,&sealevel_capi,&border_capi,&depids_capi,&pyslp_capi,&pydnodes_capi,&pyids_capi,&pyrocknb_capi))
    return NULL;
/*frompyobj*/
  ...
  if (capi_pyslp_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(pdalgo_error,"failed in converting 6th argument `pyslp' of pdalgo.marine_distribution to C/Fortran array" );
  } else {
...
#endif
/*callfortranroutine*/
        (*f2py_func)(elevation,seavol,&sealevel,border,depids,pyslp,diffsed,&pydnodes,&pyids,&pyrocknb);
if (PyErr_Occurred())
  f2py_success = 0;
#ifdef F2PY_REPORT_ATEXIT
